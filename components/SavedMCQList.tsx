...
import { exportMCQToPDF } from '../services/pdfService';
  ...
  return (
    <li className="bg-white p-6 rounded-lg shadow-md border border-slate-200 space-y-4">
      {biasWarnings.length > 0 && (
          <div className="p-3 bg-yellow-100 border-l-4 border-yellow-400 text-yellow-800 rounded-r-lg flex items-start">
              ...
              <div>
                  ...
                  <p className="text-sm">{t('biasTerms')} <span className="font-semibold">{biasWarnings.join(', ')}</span>.</p>
              </div>
          </div>
      )}
      ...
      <div className="mt-4 pt-4 border-t border-slate-200 flex justify-end space-x-3">
        ...
        <button onClick={() => exportMCQToPDF(mcq, language)} className="flex items-center px-4 py-2 text-sm font-medium text-blue-700 bg-blue-50 border border-blue-200 rounded-md hover:bg-blue-100 transition-colors">
